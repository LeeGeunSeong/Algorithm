...
public class Main_17837_새로운게임2_이근성 {
		...
		while(cnt < 4) {
			...
			for (int i = 0; i < K; i++) {
				...
				if(map[nx][ny] == 1) {
					...
					j--;
					int temp = idxMap[x][y][idx]; 
					idxMap[x][y][idx] = idxMap[x][y][j];
					idxMap[x][y][j] = temp;
				}
				// idx 이동
				for (j = 0; j < K; j++) 
					if(idxMap[nx][ny][j] == 0) break;
				for (int k = j+idx; k < j + cntMap[x][y]; k++) {
					int temp = idxMap[x][y][k-j];
					idxMap[nx][ny][k-idx] = temp;
					idxMap[x][y][k-j] = 0;
